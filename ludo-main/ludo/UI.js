import { COORDINATES_MAP, PLAYERS, STEP_LENGTH } 
from './constants.js';
...
export class UI {
    ...
    static setPiecePosition(player, piece, newPosition) {
        if(!playerPiecesElements[player] ||
            ...
            console.error(`Player element of given player: 
            ${player} and piece: ${piece} not found`)
            return;
        }
    ...
    static setTurn(index) {
        ...
        const activePlayerBase = 
        document.querySelector('.player-base.highlight');
        if(activePlayerBase) {
            activePlayerBase.classList.remove('highlight');
        }
        // highlight
        document.querySelector(`[player-id="${player}"].player-base`)
